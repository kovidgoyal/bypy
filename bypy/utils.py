...
def run(*args, **kw):
    ...
    if append_to_path:
        ...
        env["PATH"] = os.pathsep.join(env["PATH"].split(os.pathsep) + list(append_to_path))
    ...
    p = subprocess.Popen(cmd, env=env, cwd=kw.get("cwd"), stdout=stdout, stdin=stdin)
    if kw.get("stdin"):
        data = kw["stdin"]
        if isinstance(data, str):
            data = data.encode("utf-8")
...
def relocate_pkgconfig_files(prefix=PREFIX):
    for path in walk(build_dir()):
        if path.endswith(".pc"):
            if re.search(f"^prefix={prefix}$", open(path).read(), flags=re.M) is None:
...
def setup_env_for_lipo(env, use_envvars_for_lipo=False):
    if is_cross_half_of_lipo_build():
        ...
        return [
            f"--build={build}-apple-darwin",
            f"--host={host}-apple-darwin",
            f"CXXFLAGS={flags}",
            f"CFLAGS={flags}",
            f"LDFLAGS={ldflags}",
        ]
...
def simple_build(
    ...
    if configure_name:
        run(configure_name, "--prefix=" + (override_prefix or build_dir()), *configure_args, env=env, prepend_to_path=prepend_to_path)
...
def qt_build(configure_args="", for_webengine=False, dep_name="", **env):
    ...
    if iswindows:
        qcm += ".bat"
    run(qcm, "..", "-help", append_to_path=append_to_path, library_path=True)
    run(qcm, "..", "-list-features", append_to_path=append_to_path, library_path=True)
    if iswindows:
    ...
    run(
        qcm,
        "..",
        *shlex.split(configure_args.strip()),
        library_path=True,
        append_to_path=append_to_path or None,
        env=env,
        prepend_to_path=prepend_to_path or None,
    )
    ...
    if for_webengine:
        ...
        for f in walk("."):
            ...
            if ext in ("ninja", "py", "bat", "json", "sh", "cc"):
                replace_in_file(f, "ninja -C", f"ninja -j {num} -C", missing_ok=True)
...
MACH_MAGICS = (FAT_MAGIC_BE, FAT_MAGIC_LE, FAT_MAGIC_64_BE, FAT_MAGIC_64_LE, MH_MAGIC_BE, MH_MAGIC_LE, MH_MAGIC_64_BE, MH_MAGIC_64_LE)
...
def fix_install_names(m, output_dir):
    ...
    for dirpath, dirnames, filenames in os.walk(output_dir):
        for f in filenames:
            ...
            if p not in dylibs and os.path.exists(p) and (p.endswith(".dylib") or (is_macho_binary(p) and ("bin" in p.split("/") or mfunc(p)))):
                dylibs.add(p)
    for p in dylibs:
        changes = []
...
def python_install():
    ...
    if ismacos:
        ...
        framework = os.path.join(build_dir(), ddir, f"Python.framework/Versions/{major}.{minor}")
    elif iswindows:
    ...
    if ismacos and "Library" in contents:
        ...
        os.rename(os.path.join(build_dir(), "Library", "Frameworks"), os.path.join(build_dir(), ddir))
    # Handle scripts
    bdir = ""
    if ismacos:
...
def get_arches_in_binary(path):
    x = subprocess.check_output(["lipo", "-archs", path]).decode("utf-8").strip()
    return {y for y in x.split()}


def create_package(module, outpath):

    exclude = getattr(module, "pkg_exclude_names", set("doc man info test tests gtk-doc README".split()))
    if hasattr(module, "modify_excludes"):
        module.modify_excludes(exclude)
    exclude_extensions = getattr(module, "pkg_exclude_extensions", set(("pyc", "pyo", "la", "chm", "cpp", "rst", "md")))
    ...
    os.makedirs(outpath)
    check_universal_binaries = ismacos and len(UNIVERSAL_ARCHES) > 1 and not getattr(module, "allow_non_universal", False)
    dylibs = set()
    src_dir = build_dir()
    ...
    for dirpath, dirnames, filenames in os.walk(src_dir):
        ...
        def is_ok(name):
            parts = name.split("/")
            for p in parts:
                if p in exclude or ("." in p and p.rpartition(".")[-1] in exclude_extensions):
                    return False
        ...
        for f in filenames:
            ...
            if is_ok(name):
                ...
                if (
                    ...
                    and full_path not in dylibs
                    and (name.endswith(".dylib") or is_macho_binary(full_path))
                    and is_ok_to_check_universal_arches(full_path)
                ):
    ...
    for x in dylibs:
        ...
        if arches != expected:
            print(f"The file {x} is not a universal binary. Copied from {src_dir}. It only has arches: {arches}", file=sys.stderr)
            shutil.rmtree(outpath)
        ...
        if not old:
            ...
            if isinstance(old, bytes):
                ...
                if isinstance(old.pattern, str):
                    old = re.compile(old.pattern.encode("utf-8"), old.flags & ~re.UNICODE)
                nraw = old.sub(new, raw)
                pat_repr = old.pattern.decode("utf-8")
        replaced = raw != nraw
        if not replaced and not missing_ok:
            raise ValueError(f'Failed (pattern "{pat_repr}" not found) to patch: {path}')
        f.seek(0), f.truncate()
        f.write(nraw)
        return replaced
...
def windows_cmake_build(headers=None, binaries=None, libraries=None, header_dest="include", nmake_target="", make=NMAKE, extra_env_for_cmake=None, **kw):
...
def cmake_build(
    make_args=(), install_args=(), library_path=None, override_prefix=None, no_parallel=False, relocate_pkgconfig=True, append_to_path=None, env=None, **kw
):
    ...
    if not iswindows:
        make_opts = [] if no_parallel else split(MAKEOPTS)
    run(make, *(make_opts + list(make_args)), cwd="build", env=env, append_to_path=append_to_path)
    mi = [make] + list(install_args) + ["install"]
...
def meson_build(extra_cmdline="", library_path=None, needs_nasm=False, **options):
    append_to_path = ""
    if ismacos and not shutil.which(MESON):
        append_to_path = "/Library/Frameworks/Python.framework/Versions/Current/bin"
    cmd = [MESON, "setup", "--buildtype=release", f"--prefix={build_dir()}", f"--libdir={build_dir()}/lib"]
    if is_cross_half_of_lipo_build():
...
"""
    ...
    cmd.append("build")
    prepend_to_path = ""
    if needs_nasm and ismacos:
        prepend_to_path = os.path.dirname(NASM)
    run(*cmd, append_to_path=append_to_path, prepend_to_path=prepend_to_path)
...
def py_compile(basedir, optimization_level="-OO"):
    ...
    if version < 3:
        run(PYTHON, optimization_level, "-m", "compileall", "-d", "", "-f", "-q", basedir, library_path=True)
        clean_exts = ("py", "pyc")
    else:
        cmd = (PYTHON, optimization_level, "-m", "compileall", "-d", "", "-f", "-q", "-b", "-j", "0", "--invalidation-mode=unchecked-hash", basedir)
        try:
...
def get_dll_path(base, levels=1, loc=LIBDIR):
    ...
    if not candidates:
        candidates = sorted(glob.glob(os.path.join(loc, "*", pat)), reverse=True)
...
def lipo(output_dirs):
    ...
    if len(binary_collections) > 1:
        raise SystemExit("The set of binaries is different across different target architectures, cannot lipo them")
...
def setup_program_parser(pa):
    a = pa.add_argument
    a("--dont-strip", default=False, action="store_true", help="Dont strip the binaries when building")
    a(
        "--compression-level",
        default="9",
    ...
    a("--skip-tests", default=False, action="store_true", help="Skip the tests when building")
    a("--sign-installers", default=False, action="store_true", help="Sign the binary installer, needs signing keys in the VMs")
    a("--notarize", default=False, action="store_true", help="Send the app for notarization to the platform vendor")
    a("--non-interactive", default=False, action="store_true", help="Do not run a shell if building fails")
    a("--build-only", help="Build only a single extension module when building program, useful for development")
    a("--extra-program-data", help="Extra data to pass to the program specific build code")
...
def setup_dependencies_parser(p):
    ...
    p.add_argument(
        "dependencies",
        nargs="*",
        help="The dependencies to build. If none are specified missing dependencies" + " only are built. Available deps:" + " ".join(choices),
    )
