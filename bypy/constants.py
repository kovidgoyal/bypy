...
if iswindows:
    ...
    paths = [p.replace("/", os.sep) for p in vcvars_env["PATH"].split(os.pathsep)]
    ...
    if ismacos:
        LDFLAGS = worker_env["LDFLAGS"] = f"-headerpad_max_install_names -L{LIBDIR}"
        ...
        if "BYPY_DEPLOY_TARGET" in os.environ:
            worker_env["MACOSX_DEPLOYMENT_TARGET"] = os.environ["BYPY_DEPLOY_TARGET"]
        NASM = f"{BIN}/nasm"
    else:
